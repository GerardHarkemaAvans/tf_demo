...
#include "demo_gazebo/plugins/ROSProximityRayPlugin.hh"
...
void ROSProximityRayPlugin::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->statePub = this->rosnode->advertise<demo_gazebo::Proximity>(
    this->stateTopic, 1, true);
  this->stateChangePub = this->rosnode->advertise<demo_gazebo::Proximity>(
